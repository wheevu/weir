...
int run_server(unsigned port,Log&lf,Metrics&m,std::atomic<bool>&stop,unsigned workers){
 if(port==0||port>65535)return 2;int s=socket(AF_INET,SOCK_STREAM|SOCK_NONBLOCK,0);if(s<0)return 1;int yes=1;setsockopt(s,SOL_SOCKET,SO_REUSEADDR,&yes,sizeof yes);sockaddr_in a{AF_INET,htons(static_cast<std::uint16_t>(port)),INADDR_ANY);if(bind(s,reinterpret_cast<sockaddr*>(&a),sizeof a)||listen(s,64)){close(s);return 1;}int ep=epoll_create1(0),wake=eventfd(0,EFD_NONBLOCK);if(ep<0||wake<0){if(ep>=0)close(ep);if(wake>=0)close(wake);close(s);return 1;}
 epoll_event ev{EPOLLIN,static_cast<std::uint64_t>(s)};epoll_ctl(ep,EPOLL_CTL_ADD,s,&ev);epoll_event we{EPOLLIN,static_cast<std::uint64_t>(wake)};epoll_ctl(ep,EPOLL_CTL_ADD,wake,&we);
  ...
  while(!stop){int n=epoll_wait(ep,es.data(),static_cast<int>(es.size()),100);if(n<0&&errno==EINTR)continue;for(int i=0;i<n;++i){int fd=static_cast<int>(es[i].data.u64);if(fd==wake){std::uint64_t x;read(wake,&x,sizeof x);std::lock_guard l(cm);while(!done.empty()){auto[c,g,id,ok]=done.front();done.pop_front();auto active=gen.find(c);if(active!=gen.end()&&active->second==g&&out.find(c)!=out.end()){std::string msg=ok?"OK "+std::to_string(id)+"\n":"ERR persistence\n";if(out[c].size()+msg.size()>1024*1024){drop(c);continue;}out[c]+=msg;epoll_event ce{EPOLLIN|EPOLLOUT|EPOLLRDHUP,static_cast<std::uint64_t>(c)};epoll_ctl(ep,EPOLL_CTL_MOD,c,&ce);}}}else if(fd==s){for(;;){int c=accept4(s,nullptr,nullptr,SOCK_NONBLOCK);if(c<0)break;gen[c]=++next_gen[c];ps.emplace(c,Parser{});epoll_event ce{EPOLLIN|EPOLLRDHUP,static_cast<std::uint64_t>(c)};epoll_ctl(ep,EPOLL_CTL_ADD,c,&ce);}}else if(gen.count(fd)){bool dead=(es[i].events&(EPOLLERR|EPOLLHUP|EPOLLRDHUP))!=0;if(!dead&&(es[i].events&EPOLLIN)){try{std::uint8_t buf[4096];for(;;){auto r=recv(fd,buf,sizeof buf,0);if(r<=0)break;for(auto&e:ps[fd].feed(buf,static_cast<std::size_t>(r))){e.id=next++;auto g=gen[fd];e.durable_completion=[&,fd,g,id=e.id](bool ok){std::lock_guard l(cm);done.emplace_back(fd,g,id,ok);std::uint64_t x=1;write(wake,&x,sizeof x);};if(!pipe->submit(std::move(e)))drop(fd);}}}catch(const std::exception&){drop(fd);}if(!gen.count(fd))continue;}if(!dead&&(es[i].events&EPOLLOUT)){auto&w=out[fd];auto r=send(fd,w.data(),w.size(),MSG_NOSIGNAL);if(r>0)w.erase(0,static_cast<std::size_t>(r));if(w.empty()){epoll_event ce{EPOLLIN|EPOLLRDHUP,static_cast<std::uint64_t>(fd)};epoll_ctl(ep,EPOLL_CTL_MOD,fd,&ce);}}if(dead||(!out[fd].empty()&&send(fd,"",0,MSG_NOSIGNAL)<0&&errno==EPIPE))drop(fd);}}}
  }
  // Stop Pipeline callbacks before closing the wake fd they signal.
  pipe.reset();
  close(wake);close(ep);close(s);return 0;
}
int run_metrics_http(unsigned port,Metrics&m,std::atomic<bool>&stop){if(port==0||port>65535)return 2;int s=socket(AF_INET,SOCK_STREAM,0);if(s<0)return 1;int yes=1;setsockopt(s,SOL_SOCKET,SO_REUSEADDR,&yes,sizeof yes);sockaddr_in a{AF_INET,htons(static_cast<std::uint16_t>(port)),INADDR_LOOPBACK};if(bind(s,reinterpret_cast<sockaddr*>(&a),sizeof a)||listen(s,8)){close(s);return 1;}pollfd p{s,POLLIN,0};while(!stop){if(poll(&p,1,100)<1)continue;int c=accept(s,nullptr,nullptr);if(c<0)continue;auto body=m.prometheus();std::string r="HTTP/1.1 200 OK\r\nContent-Length: "+std::to_string(body.size())+"\r\nConnection: close\r\n\r\n"+body;send(c,r.data(),r.size(),MSG_NOSIGNAL);close(c);}close(s);return 0;}
#else
